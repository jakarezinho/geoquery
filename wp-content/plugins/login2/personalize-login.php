 ...
 * Version:           1.0.0
 * Author:            Jarkko Laine/jorge Melo
 * License:           GPL-2.0+
...
class Personalize_Login_Plugin {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'do_user_infos' ) );
		...
		add_shortcode( 'custom-account-info-form', array( $this, 'render_acount_info_form' ) );
	}
	...
	public static function plugin_activated() {
		...
		$page_definitions = array(
			...
			'member-account' => array(
				'title' => __( 'Your Account', 'member-account' ),
				'content' => '[custom-account-info-form]'
			),
	...
	/**
	 * A shortcode for rendering the new user acount info 
	 *ACOUNT INFO
	 *
	 * @param  array   $attributes  Shortcode attributes.
	 * @param  string  $content     The text content for shortcode. Not used.
	 ...
	 * @return string  The shortcode output
	 */
	 public function render_acount_info_form( $attributes, $content = null ) {
		// Parse shortcode attributes
	 	$default_attributes = array( 'show_title' => false );
	 	$attributes = shortcode_atts( $default_attributes, $attributes );
	 	if ( !is_user_logged_in() ) {
	 		return __( 'Tem de estar registado.', 'personalize-login' );
	 	} else {
// Retrieve possible errors from request parameters
	 		$attributes['errors'] = array();
	 		if ( isset( $_REQUEST['register-errors'] ) ) {
            ...
            if ( isset( $_REQUEST['update'] ) ) {
            	...
            	$attributes['infos']=" Infos de perfil actualizadas!";
            }

	 		return $this->get_template_html( 'acount-infos', $attributes );
	...
	//
    /**
	 * Handles the update user info 
	 *
	 */
    public function do_user_infos() {

    	if (  ! is_admin() && !isset( $_POST['manage_avatar_submit'])&& 'POST' == $_SERVER['REQUEST_METHOD']&& isset($_REQUEST['user_id'] )) {
		...
		//var_dump($_REQUEST); die();
    		...
    		if ( !is_user_logged_in() ) {
                ...
                $display_name = (isset($_POST['display_name'])) ? sanitize_text_field( $_POST['display_name']) : NULL;
                $description = (isset($_POST['description'])) ? sanitize_text_field( $_POST['description'] ) : NULL;
    			$result = $this->infos_user($user_id, $email, $first_name, $last_name, $display_name,$description );
    			...
    			if ( is_wp_error( $result ) ) {
    				...
    				$redirect_url = home_url( 'member-account' );
					$redirect_url = add_query_arg( 'update', 'infos', $redirect_url );
				}
	...
	public function do_register_user() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			} elseif ( $attributes['recaptcha_site_key']  ) {
...
/**
	 ...
	 */
private function infos_user( $user_id,$email, $first_name, $last_name, $display_name,$description ) {
	$errors = new WP_Error();
	...
	if (!empty($email)) {
		...
		if ( username_exists( $email ) || email_exists( $email ) || is_super_admin($user_id)) {
	...
	private function get_error_message( $error_code ) {
		switch ( $error_code ) {
           ...
           // user name
			...
			case 'first_name':
			return __( "Sorry, tem de escolher un nome de utilizador.", 'personalize-login' );
