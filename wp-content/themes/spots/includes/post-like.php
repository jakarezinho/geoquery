...
function getPostLikeLink( $post_id ) {
	...
	$output = '<a href="#" class="jm-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'<br>'.$count.'</a>';
	return $output;
}
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	//echo " <header><h3>" . _e( 'Artigos mais populares' ) . "</h3></header>\n";
	echo "<ul class='rating_perso'>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(
			  array(
	...
	while ( $pop_posts->have_posts() ) {
		$pop_posts->the_post();
		$post_like_count = get_post_meta( $post->ID, "_post_like_count", true ); $post_like_count ? $likes = $post_like_count : $likes = 0;
		$author = get_the_author();
		echo "<li><a href='" . get_permalink($post->ID) . "'>".get_the_post_thumbnail( $post->ID, 'thumbnail')." <span>". get_the_title()." </span></a> <br> Likes: $likes<br> By $author </li>\n";
		//echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
	}
	wp_reset_postdata();
	echo "</ul>\n";
